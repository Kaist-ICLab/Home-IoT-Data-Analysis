...
def create_model(modelname='conv1dcat', y_train=None):
    ...
    if args.criterion_type == 'weighted_bce' and y_train is not None:
        ...
        pos_weight = torch.tensor([class_counts[0] / (class_counts[1] + 1e-6)], dtype=torch.float32).to(device)
    ...
    # print("pos_weight:", pos_weight if 'pos_weight' in locals() else "N/A")
...
def train_iteration(model, train_dataset, optimizer, criterion, epoch, use_mixup=False, alpha=1.0, mixup_ratio=0.5):
    ...
    for _ in range(epoch):
        for b_idx, (X_batch, y_batch) in enumerate(train_dataset):
            ...
            # Mixup을 mixup_ratio에 따라 일부 배치에만 적용
            if use_mixup and random.random() < mixup_ratio:
                # Mixup 데이터 생성 및 손실 계산
                ...
                # Mixup 손실과 원본 손실의 평균 사용
                total_loss = (mixup_loss + original_loss) / 2
            else:
                # Mixup을 사용하지 않는 경우 원본 데이터로만 손실 계산
...
def train_test(label='phq2_result_binary', splitter='loso', modelname='dnn_s', batch_size=64, epoch=100):
    ...
    result_text = f"{args.splitter},{args.dataset},{modelname},{augment},{label},{np.mean(auc_scores):.4f}(σ={np.std(auc_scores):.4f}),{np.mean(accuracies):.4f}(σ={np.std(accuracies):.4f}),{np.mean(f1_scores):.4f}(σ={np.std(f1_scores):.4f}),{np.mean(balanced_accuracies):.4f}(σ={np.std(balanced_accuracies):.4f})"
    ...
    result_text += f",{args.batch_size},{args.epoch},{args.alpha},{args.mixup_ratio}"
